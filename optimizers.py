...
class SGD(Optimizer):
    ...
    def update(self, xs, xs_grads, iteration):
        ...
        #############################################################
            ...
            self.moments[k]=v
            #nesterov
            new_xs[k]=xs[k]+self.momentum*v-self.lr*xs_grads[k]
            #not nesterov
            #new_xs[k]=xs[k]+v
...
class Adam(Optimizer):
    ...
    def update(self, xs, xs_grads, iteration):
        ...
        t=iteration+1        
        lr_t=self.lr*(np.sqrt(1-np.power(self.beta_2,t))/(1-np.power(self.beta_1,t)))

        for k in list(xs.keys()):
        #############################################################
        # remove pass and code in for loop
        #############################################################
            m_t=(self.beta_1*self.moments[k])+(1-self.beta_1)*xs_grads[k]
            v_t=(self.beta_2*self.accumulators[k])+(1-self.beta_2)*xs_grads[k]**2
            new_xs[k]=xs[k]-lr_t*m_t/(np.sqrt(v_t)+self.epsilon)
            self.moments[k]=m_t
            self.accumulators[k]=v_t
        return new_xs
    ...
    def update(self, xs, xs_grads, iteration):
        ...
        if not self.accumulators:
            ...
            for k,v in xs.items():
                self.accumulators[k] = np.zeros(v.shape)
        for k in list(xs.keys()):
            self.accumulators[k] = self.decay * self.accumulators[k] + (1 - self.decay) * xs_grads[k]**2
            new_xs[k] = xs[k] - self.lr * xs_grads[k] / (np.sqrt(self.accumulators[k]) + self.epsilon)
        return new_xs